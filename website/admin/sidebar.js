...
function renderSidebar(activePage, role) {
  ...
  const allLinks = [
    ...
    { group: 'Conteúdo', adminOnly: true, items: [
    ...
    { group: 'Sistema', adminOnly: true, items: [
...
async function initSidebar(activePage, requiredRoles = ['admin', 'seller']) {
  const session = await AdminPortal.verificarRole(requiredRoles);
  if (!session) return null;
  const role = await AdminPortal.obterMeuRole();
  const el = document.getElementById('sidebar-mount');
  if (el) el.innerHTML = renderSidebar(activePage, role);
